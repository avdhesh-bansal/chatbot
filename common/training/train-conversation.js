...
conversation.listWorkspaces(function(err, response) {
  if (err) {
    ...
    updateEnvProperties({
      regexText: 'REPLACE_WITH_ASSISTANT_WORKSPACE_ID',
      replacement: response.workspaces[0].workspace_id,
    });
  } else {
    console.log('Creating a workspace...');
    conversation.createWorkspace(require('./common/training/conversation/car_workspace.json'), function(err, response) {
      if (!err) {
        updateEnvProperties({
          regexText: 'REPLACE_WITH_ASSISTANT_WORKSPACE_ID',
