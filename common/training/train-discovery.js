...
var discoveryV1 = new watson.discovery({ version: 'v1', version_date: '2017-11-07' });
var retryOptions = { times: 3, interval: 200 };
var defaultConfigName = 'Default Configuration with NLU'; // Use Default Configuration with NLU
var collectionName = 'CarManualsCollection';
var collectionDescription = 'Collection for Conversation with Discovery - Car Manuals';
var environmentName = 'demoEnvironment';
var environmentDescription = 'Environment for Conversation with Discovery';
...
var createConfigurationAsync = function(params, cb) {
  var configFile = fs.readFileSync('./common/training/discovery/discovery_config.json', 'utf-8'); // If you are not using default configuration, add your configuration to ./training/discovery/discovery_config.json
...
var uploadDocument = function(params, cb) {
  var wrapped = async.timeout(uploadDocumentAsync, 5000);
...
var uploadDocuments = function(params, callback) {
  ...
  var asyncTasks = [];
  fs.readdir('./common/training/discovery/carmanualdocs/', function (err, files) {
    if (err) {
      throw err;
    }
    files.map(function (file) {
      return path.join('./common/training/discovery/carmanualdocs/', file);
  ...
  function(errorConfiguration, configuration) {
    ...
    function(errorCollection, collection) {
      ...
      function(errorUpload, docs) {
        ...
        updateEnvProperties({
          regexText: 'REPLACE_WITH_DISCOVERY_ENVIRONMENT_ID',
          replacement: environment.environment_id
        });
        updateEnvProperties({
          regexText: 'REPLACE_WITH_DISCOVERY_CONFIGURATION_ID',
          replacement: configuration.configuration_id
        });
        updateEnvProperties({
          regexText: 'REPLACE_WITH_DISCOVERY_COLLECTION_ID',
